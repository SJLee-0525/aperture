...
const ic = (n, s=18)=>`<svg viewBox="0 0 24 24" width="${s}" height="${s}" fill="none" stroke="currentColor" stroke-width="1.7" style="vertical-align:middle">${ICON[n]||''}</svg>`;

const POOL = [
  ['tone01.png','새벽의 항구','f/2.8 · 1/500 · ISO100',0.80],
  ['tone02.png','골목, 5시','f/1.4 · 1/2000 · ISO64',1.20],
  ['tone03.png','안개 능선','f/8 · 1/125 · ISO200',0.74],
  ['tone04.png','부두','f/4 · 1/250 · ISO100',1.05],
  ['tone05.png','심야','f/1.8 · 1/60 · ISO800',0.82],
  ['tone06.png','설원','f/11 · 1/400 · ISO100',1.30],
  ['tone07.png','파도','f/5.6 · 1/1000 · ISO200',0.72],
  ['tone08.png','다리','f/2 · 1/320 · ISO160',1.10],
  ['tone09.png','노을','f/8 · 1/200 · ISO100',0.86],
  ['tone10.png','빗속','f/1.4 · 1/125 · ISO400',1.00],
...
function segmented(active){
  const b=(n,id,icn)=>`<button ${active===id?'aria-pressed="true"':''}>${ic(icn,15)} ${n}</button>`;
...
const chips = (arr)=>`<div class="chiprow">${arr.map((t,i)=>`<span class="chip-tag ${i===0?'active':''}">${t}</span>`).join('')}</div>`;
...
function homeA(){
  return `<div class="wf-screen">
    ...
    <div class="wf-mason">${POOL.concat(POOL.slice(0,4)).map(p=>tile(p)).join('')}</div>
  </div>`;
}

function homeB(){
  const rl=(n,id,icn)=>`<a href="#" class="${id==='work'?'cur':''}">${ic(icn,18)} ${n}</a>`;
  return `<div class="wf-screen wf-row">
    ...
    <div class="wf-main">
      ...
      <div class="wf-sq">${POOL.concat(POOL.slice(0,4)).map(p=>tile(p,'1')).join('')}</div>
    </div>
  </div>`;
}
...
function detailA(){
  const mr=(k,v)=>`<div class="wf-mr" style="border-color:rgba(255,255,255,.08)"><span class="k" style="color:#8a8a93">${k}</span><span class="v" style="color:#fafafa">${v}</span></div>`;
