...
const pic = (n, s = 18) =>
  `<svg viewBox="0 0 24 24" width="${s}" height="${s}" fill="none" stroke="currentColor" stroke-width="1.7" style="vertical-align:middle">${P_ICON[n] || ""}</svg>`;
...
function framePreview(style, p, o) {
  ...
  const wm = o.wm
    ? `<div class="fp-wm">Aperture<span style="color:var(--accent)">.</span></div>`
    : "";
  const camShort = p.cam.replace(/^(Sony|Fujifilm|Nikon|Leica)\s/, "");
  const ph = (ar, extra = "") =>
    `<div class="fp-ph" style="width:100%;aspect-ratio:${ar};${extra}"><div class="fp-img" style="background-image:url(${url})"></div>${wm}</div>`;
  switch (style) {
    case "bar":
      return `<div class="fr-bar">${ph("4/5")}<div class="bar"><span class="cam">${camShort} · ${p.fl}</span><span class="ex">${metaLine(p, o.range)}</span></div></div>`;
    case "pola":
      return `<div class="fr-pola">${ph("1")}<div class="cap"><div class="ttl">${p.title}</div><div class="ex">${metaLine(p, o.range)}</div></div></div>`;
    case "film":
      return `<div class="fr-film"><div class="holes"></div><div class="reb"><span class="t">HSW 400 · ${camShort}</span><span class="no">32A</span></div>${ph("3/2")}<div class="reb"><span class="t">${p.f} · ${p.s} · ISO${p.iso} · ${p.fl}</span><span class="no">»</span></div><div class="holes"></div></div>`;
    case "mat":
      return `<div class="fr-mat">${ph("3/2", "border:1px solid var(--line-strong)")}<div class="plate"><div class="ttl">${p.title}</div><div class="meta">${camShort} — ${metaLine(p, o.range)}</div></div></div>`;
    case "corner":
      return `<div class="fr-corner">${ph("4/5")}<div class="ov"><div class="big">${p.f} · ${p.s} · ISO ${p.iso}</div><div class="sm">${o.range === "loc" ? p.place : p.cam + " · " + p.lens}</div></div></div>`;
    case "side":
      return `<div class="fr-side"><div class="fp-ph" style="width:62%;flex:none;aspect-ratio:3/4"><div class="fp-img" style="background-image:url(${url})"></div>${wm}</div><div class="panel"><div><div class="k">Aperture</div><div class="v">${p.f}</div></div><div><div class="k">Shutter</div><div class="v">${p.s}</div></div><div><div class="k">ISO</div><div class="v">${p.iso}</div></div><div><div class="k">Camera</div><div class="v">${camShort}</div></div></div></div>`;
  }
  return "";
}
...
function renderMap() {
  const pins = PHOTOS.map(
    (p, i) =>
      `<button class="map-pin2" data-i="${i}" style="left:${MAP_POS[i][0]}%;top:${MAP_POS[i][1]}%"><span class="mp-dot"></span><span class="mp-lab">${p.place.split(" ")[0]}</span></button>`,
  ).join("");
  const list = PHOTOS.map(
    (p, i) => `<button class="loc-item" data-i="${i}">
...
function renderTagbar(active) {
  return allTags()
    .map(
      (t) =>
        `<button class="chip-tag ${t === active ? "active" : ""}" data-tag="${t}">${t}</button>`,
    )
    .join("");
}
function camOptions() {
  return ["전체", ...new Set(PHOTOS.map((p) => p.cam))]
...
function filteredIndexes(tag, q, cam, fMin, fMax) {
  ...
  return PHOTOS.map((_, i) => i).filter((i) => {
    ...
    if (q) {
      const hay = [p.title, p.cam, p.lens, p.place, p.tags.join(" ")].join(" ").toLowerCase();
      if (!hay.includes(q)) return false;
    }
    return true;
  });
}
function renderGrid(idxs) {
  return idxs.length
    ? idxs.map((i) => tileHTML(i)).join("")
    : '<div class="empty-grid">검색 결과가 없습니다</div>';
}
...
function renderAbout() {
  ...
  const stat = (n, l) =>
    `<div class="stat"><div class="sn">${n}</div><div class="sl">${l}</div></div>`;
  const block = (label, items) =>
    `<div class="about-block"><div class="u-label">${label}</div><ul class="about-list">${items.map((x) => `<li>${x}</li>`).join("")}</ul></div>`;
