...
(function () {
  const WORKS = [
    {
      ...
      desc: "슈베르트 겨울 나그네 전곡. 24개의 가곡을 하나의 긴 호흡으로.",
    },
    {
      t: "Chopin Recital",
      sub: "Chopin",
      ...
      desc: "쇼팽의 발라드와 스케르초를 중심으로 한 프로그램.",
    },
    {
      t: "Rachmaninoff No.2",
      sub: "with KBS Symphony",
      d: "2025.06.20",
      time: "20:00",
      v: "세종문화회관 대극장",
      tag: "협연",
      program: ["Piano Concerto No.2 in C minor, Op.18"],
      desc: "라흐마니노프 피아노 협주곡 2번. KBS 교향악단과 협연.",
    },
    {
      t: "Schubert Sonatas",
      sub: "Schubert",
    ...
    {
      t: "Debussy · Ravel",
      sub: "French Programme",
      d: "2024.05.19",
      time: "17:00",
      v: "금호아트홀",
      tag: "리사이틀",
      program: ["Debussy · Estampes", "Ravel · Gaspard de la nuit"],
      desc: "프랑스 인상주의 프로그램.",
    },
    {
      ...
      desc: "바흐 골드베르크 변주곡 전곡.",
    },
    {
      t: "Mozart Concertos",
      sub: "with Korean Chamber",
      d: "2023.11.23",
      time: "20:00",
      v: "롯데콘서트홀",
      tag: "협연",
      program: ["Concerto No.20 in D minor, K.466", "Concerto No.23 in A, K.488"],
      desc: "모차르트 협주곡 두 곡.",
    },
  ];
  const SCHEDULE = [
    ...
    },
    {
      d: "08.03",
      y: "2026",
      t: "Beethoven Concerto No.5 “Emperor”",
      v: "롯데콘서트홀",
      status: "예매 중",
      on: true,
    },
  ...
  const AWARDS = [
    {
      ...
      n: "국제 피아노 콩쿠르 — 파이널리스트",
      place: "Geneva, CH",
      desc: "세계적 권위의 콩쿠르 결선에 올라 협주곡 라운드를 연주했다.",
    },
    {
      y: "2022",
      n: "부조니 콩쿠르 — 2위",
      place: "Bolzano, IT",
      desc: "부조니 국제 피아노 콩쿠르에서 2위와 함께 청중상을 받았다.",
    },
    {
      y: "2021",
      n: "리즈 콩쿠르 — 세미파이널",
      place: "Leeds, UK",
      desc: "리즈 국제 피아노 콩쿠르 준결선에 진출했다.",
    },
    {
      y: "2020",
      n: "서울 음악 콩쿠르 — 1위",
      place: "Seoul, KR",
      desc: "국내 최정상 콩쿠르에서 1위를 차지했다.",
    },
    {
      y: "2019",
      n: "한국 청년 음악가상",
      place: "Seoul, KR",
      desc: "그해 가장 주목받은 신예 음악가에게 주어지는 상을 받았다.",
    },
    {
      y: "2018",
      n: "영 콘서트 아티스트 — 수상",
      place: "New York, US",
      desc: "국제 오디션을 통해 북미 데뷔 무대 기회를 얻었다.",
    },
  ];
  const VIDEOS = [
  ...
  const AR =
    '<svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M7 17L17 7M9 7h8v8"/></svg>';
  ...
  function startType(root) {
    ...
    typedStarted = true;
    const el = root.querySelector("#mu-typed");
    if (!el) return;
    let wi = 0,
      ci = 0,
      del = false;
  ...
  function init(root) {
    ...
    root.addEventListener("click", (e) => {
      ...
      if (w) {
        openModal(recitalHTML(WORKS[+w.dataset.work]));
        return;
      }
      const a = e.target.closest("[data-award]");
      if (a) {
        openModal(awardHTML(AWARDS[+a.dataset.award]));
        return;
      }
      const v = e.target.closest("[data-video]");
      if (v) {
        const idx = +v.dataset.video;
        const vid = VIDEOS[idx];
        v.innerHTML = vid.id
          ? `<iframe src="https://www.youtube.com/embed/${vid.id}?autoplay=1" allow="autoplay; encrypted-media" allowfullscreen></iframe>`
          : `<div class="facade" style="background:#111"><div class="vt">${vid.t}</div><div class="vs">유튜브 영상 ID를 연결하면 여기서 재생됩니다</div></div>`;
        return;
      }
      const sc = e.target.closest("[data-muscroll]");
      if (sc) {
        e.preventDefault();
        nav(sc.dataset.muscroll, root);
        return;
      }
    });
  ...
  function nav(sub, root) {
    ...
    const el = root.querySelector("#" + (map[sub] || "mu-top"));
