...
const mi = (n, s = 20) =>
  `<svg viewBox="0 0 24 24" width="${s}" height="${s}" fill="none" stroke="currentColor" stroke-width="1.7">${MI[n] || ""}</svg>`;

function statusBar(onPhoto) {
  const ic =
    `<span class="si"><svg width="17" height="11" viewBox="0 0 17 11" fill="currentColor"><rect x="0" y="6" width="3" height="5" rx="1"/><rect x="4" y="4" width="3" height="7" rx="1"/><rect x="8" y="2" width="3" height="9" rx="1"/><rect x="12" y="0" width="3" height="11" rx="1"/></svg>` +
...
function tabbar(active) {
  const t = (n, id, icn) =>
    `<div class="m-tab ${id === active ? "on" : ""}" data-tab="${id}">${mi(icn, 22)}<span>${n}</span></div>`;
...
function mAlbums() {
  const card = (a, i) =>
    `<figure class="m-alb" data-album="${i}"><div class="cov"><div class="ph" style="background-image:url(images/${a.cover})"></div><span class="cnt">${a.photos.length}</span></div><div class="ti">${a.title}</div><div class="me">${a.sub}</div></figure>`;
  return `<div class="m-screen">${statusBar()}
...
function mAlbumDetail(i = 0) {
  ...
  return `<div class="m-screen">${statusBar(true)}
    ...
    <div class="m-body"><div class="m-grid" style="padding-top:12px">${a.photos
      .concat(a.photos.slice(0, 2))
      .map((i) => mTile(i))
      .join("")}</div></div>
    ${tabbar("albums")}</div>`;
}
...
function mMap() {
  ...
  const pins = pos
    .map(
      (q, i) => `<div class="pin" style="left:${q[0]}%;top:${q[1]}%"><div class="dot"></div></div>`,
    )
    .join("");
  const locs = [0, 3, 6, 8, 11]
    .map((i) => {
      const p = PHOTOS[i];
      return `<div class="m-loc"><div class="th" style="background-image:url(images/${p.img})"></div><div class="pl">${p.place}</div><div class="co">${p.coords}</div></div>`;
    })
    .join("");
  return `<div class="m-screen">${statusBar()}
...
function mAbout() {
  const cams = [...new Set(PHOTOS.map((p) => p.cam))],
    places = [...new Set(PHOTOS.map((p) => p.place.split(" ")[0]))];
  const st = (n, l) =>
...
function mExportSheet(i = 0) {
  ...
  const items = styles
    .map(
      ([s, nm], i) =>
        `<div class="m-expitem ${i === 0 ? "on" : ""}"><div class="fr-wrap">${framePreview(s, p, { wm: false, range: "expo", res: "orig" })}</div><div class="nm">${nm}</div></div>`,
    )
    .join("");
