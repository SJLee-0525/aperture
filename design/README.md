...
- **음악·개발**은 개별 페이지 네비. 개발은 기술 스택(`/dev`)·프로젝트(`/dev/projects`)·경력(`/dev/career`)으로 나뉘며 프로젝트 상세는 `?project=` 모달이다.
...
| 9   | 랜딩 개발 행 = 개발 섹션 루트 진입              | 대표 콘텐츠인 **프로젝트 목록(`/dev/projects`)으로 직행**. `/dev`는 기술 스택 페이지로 유지                                           | 사용자 확정 (랜딩에서 프로젝트 우선 노출)             |
