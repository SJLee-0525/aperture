...
| 10  | 태그 칩 행 `.chiprow` = 여러 줄 wrap            | **어느 폭에서든 한 줄 + 좌우 스크롤**, 넘치는 방향에만 화살표 (`components/TagFilterBar`)                                             | 사용자 확정 (태그가 늘어도 목록을 아래로 밀지 않는다) |
| 11  | (명시 없음)                                     | 목록 결과 수는 두 언어에서 같은 영어 표기 (`12 photos` · `7 spots` · `9 articles`)                                                    | 사용자 확정 (숫자+단위는 언어 무관 표기)              |
