 ...
 *
 * 글 복구본에는 아직 저장하지 않은 본문 Markdown 전체가 들어 있다. 이 값은 mock 분기 없이
 * 실데이터 모드에서도 저장되므로(`use-article-recovery`), 공용 브라우저에서 로그아웃한 뒤에도
 * 미발행 글이 그대로 남는다.
 *
...
const isDraftKey = (key: string): boolean => key.startsWith(ADMIN_DEV_ARTICLE_DRAFT_KEY_PREFIX);
