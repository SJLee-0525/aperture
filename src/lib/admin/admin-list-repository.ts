 ...
 *
 * 화면(훅·페이지)은 이 타입만 보고, mock(브라우저 로컬)과 live(Supabase)
 * 구현이 같은 모양을 강제받는다. "화면은 저장소의 정체를 모른다" 는 경계를 관리자
 * 도메인 전체가 공유한다. `list` 는 목록 행 투영만, `get` 은 편집에
 * 필요한 전체 문서를 돌려준다. 저장소 구현이 바뀌어도 읽는 양이 그대로여야 한다.
 */
