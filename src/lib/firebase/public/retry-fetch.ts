 ...
 *
 * 429 는 재시도하지 않는다. Firestore 의 429 는 무료 한도 소진(`RESOURCE_EXHAUSTED`)이 대부분이고
 * 그 상태는 한도 창이 바뀔 때까지 이어진다. 다시 보내면 남은 한도만 더 쓴다.
 */
const isRetriableStatus = (status: number): boolean => status >= 500;

/**
 * Firestore REST 읽기를 네트워크 오류와 일시적 상태 코드에 한해 다시 시도한다.
 *
 * 정적 생성은 페이지 하나의 읽기가 실패하면 빌드 전체를 중단한다. 빌드 컨테이너에서
 * `firestore.googleapis.com` 연결이 한 번 ETIMEDOUT 되는 것만으로 배포가 깨지지 않게 한다.
