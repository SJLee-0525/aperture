 ...
 *
 * 관리자 화면은 Supabase Auth 로 들어간다. 자동화 테스트에 실제 계정을 쓰면 비밀번호를 저장소나
 * CI 에 두게 되고, `NEXT_PUBLIC_USE_MOCK=1` 만으로는 `AuthGuard` 를 지나갈 수 없다. 그래서
 * mock 콘텐츠와 별개로 인증 경계에 주입하는 스위치를 둔다. E2E 와 Supabase 자격증명 없는 로컬
 * 개발이 같은 플래그를 쓰며, mock 콘텐츠 여부만으로 인증이 열리는 일은 없다.
 *
