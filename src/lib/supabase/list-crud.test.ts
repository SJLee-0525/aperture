...
import { documentCrud, sortableListCrud, type PostSyncPolicy } from "@/lib/supabase/list-crud";
...
describe("documentCrud 는 목록·정렬 연산을 노출하지 않는다", () => {
  it("정렬이 없는 컬렉션에는 updateOrder·list 가 타입에 없다", () => {
    const crud = documentCrud<Entity>("devArticles", toEntity, "글");

    // 런타임 검사가 아니라 타입 검사다. 두 지시자가 만족되지 않으면 tsc 가 실패한다.
    // @ts-expect-error devArticles 에는 수동 정렬이 없어 정렬 RPC 도 없다.
    void crud.updateOrder;
    // @ts-expect-error 서술자 select 가 본문을 포함해, 목록 조회는 모든 글의 Markdown 을 끌어온다.
    void crud.list;

    expect(Object.keys(crud)).not.toContain("updateOrder");
    expect(Object.keys(crud)).not.toContain("list");
  });
});

describe("documentCrud/sortableListCrud — 정책 미주입 (기존 컬렉션 경로)", () => {
  it("네 가지 쓰기 모두 동기화를 요청하고 쓰기 직전 스냅샷은 읽지 않는다", async () => {
    const crud = sortableListCrud<Entity>("devProjects", toEntity, "프로젝트", "project");
...
describe("documentCrud/sortableListCrud — 정책 주입", () => {
  it("skip 을 돌려주면 동기화를 요청하지 않는다", async () => {
    const policy = vi.fn<PostSyncPolicy<Entity>>().mockReturnValue("skip");
    const crud = documentCrud<Entity>("devArticles", toEntity, "글", "project", policy);
  ...
  it("ragSourceType 이 없으면 정책 결과와 무관하게 요청과 스냅샷 조회가 없다", async () => {
    const policy = vi.fn<PostSyncPolicy<Entity>>().mockReturnValue("sync");
    const crud = documentCrud<Entity>("devArticles", toEntity, "글", undefined, policy);
...
describe("documentCrud/sortableListCrud — supabase 오류·0행 처리", () => {
...
describe("documentCrud/sortableListCrud — 일괄 정렬", () => {
