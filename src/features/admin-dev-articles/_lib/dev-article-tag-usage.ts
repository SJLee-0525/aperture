 ...
 *
 * @param {Array<{ tags: string[] }>} articles 검사할 글 목록. 초안을 포함해야 한다 —
 *   초안만 쓰는 태그를 지우면 그 초안이 발행 조건(`tag-unknown`)에 걸린다.
 * @param {string} tagId 셀 태그 id.
 * @returns {number} 이 태그를 참조하는 글 수.
 */
 ...
 *
 * @param {number} count 이 태그를 참조하는 글 수.
 * @returns {string} 거부 사유와 다음 행동을 담은 문구.
 */
