 ...
 *
 * @param {AdminDevArticleListItem} item 검사할 목록 행.
 * @param {string} keyword 소문자로 맞춘 검색어.
 * @returns {boolean} 걸리면 true.
 */
 ...
 * 상태 필터와 검색어를 적용한다.
 *
 * @param {AdminDevArticleListItem[]} items 정렬된 목록.
 * @param {{ status: AdminArticleStatusFilter; keyword: string }} query 필터 조건.
 * @param {AdminArticleStatusFilter} query.status 초안·발행·전체.
 * @param {string} query.keyword 제목·주소에서 찾을 문자열. 공백만 있으면 검색하지 않는다.
 * @returns {AdminDevArticleListItem[]} 조건을 만족하는 행.
 */
