...
describe("validateAwardInput", () => {
  it("한국어 수상명을 먼저 요구한다", () => {
    expect(validateAwardInput(emptyAwardInput())).toContainEqual(
      expect.objectContaining({ message: "수상명(한국어)을 입력하세요." }),
    );
  });
  ...
  it("연도를 비우면 저장할 수 없다", () => {
    ...
    expect(validateAwardInput(input)).toContainEqual(
      expect.objectContaining({ message: "연도를 입력하세요." }),
    );
  });
