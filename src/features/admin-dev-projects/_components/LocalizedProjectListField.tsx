...
type Props = {
  /** 편집할 프로젝트 배열 필드. */
  /** 편집할 배열 필드의 상태 키. `AdminField` 의 검증용 `field` 와 다른 개념이다. */
  arrayKey: LocalizedArrayKey;
  /** 필드 위에 표시할 제목. */
  legend: string;
  ...
  /** 지정한 필드에 빈 항목을 추가한다. */
  onAdd: (arrayKey: LocalizedArrayKey) => void;
  /** 지정한 항목의 언어별 문자열을 수정한다. */
  onEdit: (arrayKey: LocalizedArrayKey, index: number, lang: "ko" | "en", value: string) => void;
  /** 지정한 필드에서 항목 하나를 제거한다. */
  onRemove: (arrayKey: LocalizedArrayKey, index: number) => void;
};
...
const LocalizedProjectListField = ({
  ...
  <section className={base.section}>
    <div className={styles.arrayHead}>
      ...
      <AdminButton variant="secondary" size="xs" onClick={() => onAdd(arrayKey)}>
        {addLabel}
      </AdminButton>
    </div>
    {items.length === 0 ? (
          ...
          <li key={index} className={styles.arrayRow}>
            <div className={`${base.grid2} ${styles.arrayGrid}`}>
              <AdminInput
                ...
                value={item.ko}
                placeholder="한국어"
                onChange={(event) => onEdit(arrayKey, index, "ko", event.target.value)}
              />
              <AdminInput
                aria-label="English"
                value={item.en}
