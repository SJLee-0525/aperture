...
describe("validateAlbumInput", () => {
  it("한국어 제목을 먼저 요구한다", () => {
    expect(validateAlbumInput(emptyAlbumInput())).toContainEqual(
  ...
  it("제목이 있으면 사진을 최소 한 장 요구한다", () => {
    ...
    expect(validateAlbumInput(input)).toContainEqual(
      expect.objectContaining({ message: "앨범에 넣을 사진을 최소 한 장 이상 선택하세요." }),
    );
  });
