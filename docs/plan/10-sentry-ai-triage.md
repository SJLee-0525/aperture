...
| claim 응답 해석 실패   | 202  | 기본 카드                  | 없음 (중복과 구분해 failed 로 본다)              |
...
| 제한기 예외            | 202  | 판정 카드 (상한 없이 진행) | 정상 기록                                        |
| 기록 시크릿 미설정     | 202  | 기본 카드 (판정 건너뜀)    | 없음. 설정 오류로 보고 `console.error`           |
| Supabase claim 실패    | 202  | 기본 카드 (판정 건너뜀)    | 없음 (행이 만들어지지 않았다)                    |
...
**선점하지 못한 전달은 판정을 건너뛴다.** 그 구간에는 같은 전달을 두 번 처리하는 것을 막을
수단이 없어, 재전송이나 재생이 매번 유료 호출로 이어진다. 카드는 그대로 보내고 판정만 포기한다.
이때 `skipped` 는 카드 사유로만 존재한다. 행이 없으므로 `triage_status` 를 쓸 대상도 없고
`complete` 도 호출되지 않는다.

**제한기 예외는 상한 없이 통과시킨다.** 제한기 자신이 fail-open 이므로 호출부도 같은 정책을 쓴다.
이 실패를 판정 실패로 처리하면 제공자를 부르지도 못한 채 기본 카드가 나간다.
