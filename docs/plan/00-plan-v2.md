...
개발 메뉴를 `소개 → 경력·기술 → 프로젝트 → 블로그`로 재구성하고, 관리자 Markdown 작성·공개 상세 페이지·프로젝트 양방향 연결·검색·RAG·화면 문맥 챗봇·WebMCP를 추가한다. 경로와 데이터 모델, 미디어·보안 계약 및 구현 순서는 [07-dev-blog.md](07-dev-blog.md)를 따른다.
