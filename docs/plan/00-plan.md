# Aperture. 구현 마스터 플랜 (00 · v1 — 사진 전용)

> 🗂️ **이 문서는 v1(사진 전용 `Aperture.`) 로드맵이며 P1·P2가 대부분 완료된 기록이다.**
> 통합 포트폴리오(사진·음악·개발)로의 확장 계획은 **[`00-plan-v2.md`](00-plan-v2.md)** 를 따른다.
> 아래 내용 중 익명 좋아요는 후속 보안·비용 검토로 제거됐다. 현재 결정은 `CLAUDE.md`와 v2 계획을 따른다.
