...
새 코드에서 두 건(C12·C13)이 나왔다. 줄번호는 전부 `84509d7` 기준이다.

## 높음

### C1. 빈 목록이 Discord 400 을 만들고 알림이 사라진다

`src/lib/performance-alerts/discord-report.ts:164-168`
...
`triage-schema.ts` 의 `itemArray()` 에 `minItems` 가 없고, `textArray` 는 길이 상한만 보므로
빈 배열이 통과한다. 두 배열이 모두 비면 `join` 결과가 빈 문자열이 되고,
Discord embed 의 field `value` 는 1자 이상이 필수라 400 이 떨어진다.

`send-webhook.ts:99-101` 은 400 을 "같은 요청을 다시 보내도 같은 답이 온다"로 보고
재시도하지 않는다. `core-web-vitals-report.ts:156-161` 이 그 실패를 받아 throw 하므로,
**경고가 전달되지 않은 채 실행이 실패한다.**

같은 파일의 `원인 후보` 는 `"근거 부족"` 폴백을 갖고, `sentry-triage/discord-card.ts:98` 은
`...(actions ? [{ name: "권장 조치", ... }] : [])` 로 같은 상황을 막는다.
`84509d7` 이 추가한 통합 카드의 `mergedChecks`(112-119줄)도 `"제안 없음"` 폴백을 갖는다.
단일 대상 경로만 남았다.

이 경로(`attachPerformanceTriage`)는 대상이 하나일 때만 쓰인다. 사이트 전역이 아니라
한 페이지가 회귀하는, 가장 흔한 경우다.

`discord-report.test.ts` 는 항목이 1개인 경우와 4개인 경우를 덮지만 0개는 덮지 않는다.
